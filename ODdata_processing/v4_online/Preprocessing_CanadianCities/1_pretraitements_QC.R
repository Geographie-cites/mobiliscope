...
# on prend le mode 2 en priorité, puis 1 puis 3 
# (véhicule privé motorisé :2 / transport public : 1 / mobilités douces : 3)

BD_brute <- BD_brute %>% 
  mutate(modep = case_when(mode1 == "2" | mode2 == "2" | mode3 == "2" |
...
BD_brute <- BD_brute %>% 
  mutate(hrearv = case_when(hrearv=="2899" ~ "2800",
                            hrearv=="2799" ~ "2800",
                            TRUE ~ hrearv))

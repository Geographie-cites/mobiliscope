...
#       Ajout d'une boucle de remplissage de la table en Mai 2019 (EV)
# 
# septembre 2018 - AD
# ================================================================================#
...
# Fonction de création de la table de présence pour les enquêtes comportant entre 12000 et 15000 personnes en déplacement
...
createPrez <- function(libED){
  ...
  VecID <- unique(tripTable_GRP$ID_IND)
    ...
    n = length(VecID)
    i = as.integer(n/3000) + 1
    
    for (j in (1:i)){
      
      VecIDj <- VecID[((j-1)*3000 + 1) : (j*3000)]
      VecIDj <- VecIDj[!is.na(VecIDj)] 
      
      prezTable_j <- data.frame(ID_IND = character (0), ID_ED = character(0), LIB_ED = character(0), ID_ORDRE = integer(0), W_IND = numeric(0),
                                CODE_ZF = character(0), CODE_COM = character(0), CODE_SEC = character(0),
                                HEURE_DEB = character(0), HEURE_FIN = character(0), DUREE = numeric(0),
                                MOTIF = integer(0), ADH_ARR = integer(0), ADH_DEP = integer(0),
                                MODE_ARR = character(0), MODE_DEP = character (0), ZONAGE = character (0), stringsAsFactors = FALSE)
      
      invisible(foreach (ind = VecIDj, .verbose = FALSE) %do% {
        ...
        prezTable_j <-rbind(prezTable_j, prez_ind)
        
      })
      
      prezTable <- rbind(prezTable, prezTable_j)
    }
  ...
  prezNonDepl <- prezNonDepl %>% 
    transmute(ID_IND, ID_ED, LIB_ED, ID_ORDRE = 1, W_IND = as.numeric(W_IND), CODE_ZF = as.character(CODE_ZF), CODE_COM = as.character(CODE_COM), CODE_SEC = as.character(CODE_SEC), 
              HEURE_DEB = as.character.Date(ISOdatetime(2010,1,1,4,0,0)), HEURE_FIN = as.character.Date(ISOdatetime(2010,1,2,4,0,0)),
              DUREE = 24*60, MOTIF = 1, ADH_ARR = 0, ADH_DEP = 0, MODE_ARR = NA, MODE_DEP = NA, ZONAGE)
  ...
  prezEnDepl <-  prezEnDepl %>% 
    transmute(ID_IND = tripTable$ID_IND, ID_ED = tripTable$ID_ED, LIB_ED = tripTable$LIB_ED, ID_ORDRE = 0, 
              W_IND = tripTable$W_IND, CODE_ZF = character(nobs), CODE_COM ="88888", CODE_SEC ="888", 
  ...
  prezTable <- left_join(prezTable, y = select(indTable, ID_IND, SEX, KAGE, REVENU, OCC, RES_SEC, RES_COG), by = "ID_IND")
  ...
  # 6. Sauvegarde
  save(prezTable, file = paste("scriptsr/data/table_presence_", nomEnq, ".RDS", sep = ""))
